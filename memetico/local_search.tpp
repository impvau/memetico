...
double local_search::custom_nelder_mead_redo(U* model, DataSet* data, vector<size_t>& selected) {
    ...
    while ( simplex.begin()->first - (--simplex.end())->first > numeric_limits<double>::epsilon()  &&   // The range in the simplex points has a difference > some epsilon   
        ...
        //cout << "Best LS:" << (--simplex.end())->first << " " << vb << " Worst LS:" << vw_fit << " " << vw << " vr_fit:" << vr_fit << " " << vr << " cent: " << cent << endl;
        ...
        if (vr_fit < vw_fit) {
            ...
            if (local_search::model_evaluate(ve, positions, model, data, selected) < cent_fit) {      
                if (vr_fit < cent_fit) {
                    vtmp = contr(vr, ve);
                    //cout << "contr(vr, ve);" << endl;
                } else {
                    vtmp = contr(refl(cent, vr), cent);
                    //cout << "contr(refl(cent, vr), cent);" << endl;
                }
            } else {
                vtmp = cent;
                //cout << "cent" << endl;
            ...
            //cout << "contr(contr(vtmp, cent), ve);" << endl;
 ...
 * 
 */
vector<double> local_search::vm(vector<double> a, double b) {
    vector<double> ret(a.size());
 ...
 * 
 */
vector<double> local_search::refl(vector<double> a, vector<double> b) {
    //cout << "Plus: " << a << b << vo(a,b,plus<double>() << " Multi2:" << vm(vo(a,b,plus<double>()),2) << endl;
    //return vm(vo(a,b,plus<double>()),2);
 ...
 * 
 */
vector<double> local_search::expa(vector<double> a, vector<double> b) {
 ...
 * 
 */
vector<double> local_search::contr(vector<double> a, vector<double> b) {
